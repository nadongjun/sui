...
fn module(
    ...
    if !genums.is_empty() {
        for (name, _) in genums {
            let mut diag = diag!(Editions::FeatureInDevelopment, (name.loc(), "Enums are not supported in bytecode."));
...
fn command(context: &mut Context, code: &mut IR::BytecodeBlock, sp!(loc, cmd_): H::Command) {
    ...
    match cmd_ {
        ...
        C::VariantSwitch { .. } => {
            let mut diag = diag!(Editions::FeatureInDevelopment, (loc, "Variant switching is not supported in bytecode."));
            diag.add_note("This feature is currently in development.");
...
fn lvalue(context: &mut Context, code: &mut IR::BytecodeBlock, sp!(loc, l_): H::LValue) {
    ...
    match l_ {
        ...
        L::UnpackVariant(..) => {
            let mut diag = diag!(Editions::FeatureInDevelopment, (loc, "Switching is not supported in bytecode."));
            diag.add_note("This feature is currently in development.");
            context.env.add_diag(diag);
        }
    }
...
fn exp(context: &mut Context, code: &mut IR::BytecodeBlock, e: H::Exp) {
    ...
    match e_ {
        ...
        E::PackVariant(..) => {
            let mut diag = diag!(Editions::FeatureInDevelopment, (loc, "Variant packing is not supported in bytecode."));
            diag.add_note("This feature is currently in development.");
            context.env.add_diag(diag);
        }

...
use sui_types::{
    ...
    storage::{GetSharedLocks, ObjectKey, SharedLocksKey},
    transaction::{
            ...
            match input {
                InputObjectKind::MovePackage(id) => {
                    let package = self.cache.get_package_object(id)?.unwrap_or_else(|| {
                        panic!("Executable transaction {shared_locks_key:?} depends on non-existent package {id:?}")
                    });
            ...
            results[index] = Some(match (object, input) {
                ...
                (None, InputObjectKind::SharedMoveObject { id, .. }) => {
                    ...
                    if let Some(dependency) = self.cache.get_deleted_shared_object_previous_tx_digest(id, &version, epoch_id)? {
                        ...
                        panic!("All dependencies of tx {shared_locks_key:?} should have been executed now, but Shared Object id: {}, version: {version} is absent in epoch {epoch_id}", *id);
                    }
                },
                _ => panic!("All dependencies of tx {shared_locks_key:?} should have been executed now, but obj {key:?} is absent"),
            });
        }

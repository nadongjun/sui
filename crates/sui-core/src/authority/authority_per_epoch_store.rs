...
use sui_types::storage::{InputKey, SharedLocksKey};
...
pub struct AuthorityEpochTables {
    ...
    assigned_shared_object_versions_v2: DBMap<SharedLocksKey, Vec<(ObjectID, SequenceNumber)>>,
...
impl AuthorityPerEpochStore {
    ...
    pub fn set_shared_object_versions_for_testing(
        ...
        if self.randomness_state_enabled() {
            self.tables()?
                ...
                .insert(&SharedLocksKey::Digest(*tx_digest), assigned_versions)?;
        } else {
            self.tables()?
    ...
    async fn set_assigned_shared_object_versions(
        ...
        let shared_locks_key = certificate.shared_locks_key();
        debug!(
            ?shared_locks_key,
            ?assigned_versions,
        ...
        if self.randomness_state_enabled() {
            self.tables()?
                .assigned_shared_object_versions_v2
                .insert(&shared_locks_key, assigned_versions)?;
        } else {
            let SharedLocksKey::Digest(digest) = shared_locks_key else {
    ...
    fn finish_assign_shared_object_versions(
        ...
        if self.randomness_state_enabled() {
            write_batch.insert_batch(
                ...
                iter::once((shared_locks_key, assigned_versions)),
            )?;
        } else {
            let SharedLocksKey::Digest(digest) = shared_locks_key else {
    ...
    pub(crate) async fn process_consensus_transactions<C: CheckpointServiceNotify>(
        ...
        let mut shared_input_next_versions = {
            let mut generate_randomness = false;
            ...
            if generate_randomness {
                ...
                batch.insert_batch(
                    &self.tables()?.assigned_shared_object_versions_v2,
                    iter::once((
                        SharedLocksKey::RandomnessRound(round),
...
impl GetSharedLocks for AuthorityPerEpochStore {
    fn get_shared_locks(
        ...
        if self.randomness_state_enabled() {
            ...
            let SharedLocksKey::Digest(digest) = key else {
                // TODO-DNS is this safe?
                panic!("should never see a SharedLocksKey::RandomnessRound when randomness is not enabled");
            };
            Ok(self
